...
#
# Based on original from Nils Knieling - https://github.com/Cyclenerd/ethereum_nvidia_miner
...
function check_internet() {
	MY_HTTP_PING_URL="https://raw.githubusercontent.com/ricardovelero/nvidia_miner/master/README.md"
...
MY_CLAYMORE_UPDATE_TITLE="Update Claymore Miner"

MY_CLAYMORE_UPDATE_MSG_TEXT="Update to latest Claymore Miner"

function my_claymore_update() {

	cmd=(dialog --backtitle "$MY_CLAYMORE_UPDATE_TITLE" --inputbox "$MY_CLAYMORE_UPDATE_MSG_TEXT" 14 60 "$MY_CLAYMORE")
	choices=$("${cmd[@]}" 2>&1 >/dev/tty)
	if [ "$choices" != "" ]; then
		MY_CLAYMORE=$choices
...
MY_MINER_TITLE="Configure Miner"
...
function my_visudo() {
	clear
	echo 'prospector ALL=(ALL) NOPASSWD: ALL' | sudo EDITOR='tee -a' visudo
	clear
}
...
if [ -f ~/settings.conf ]; then
	...
	curl -f "https://raw.githubusercontent.com/ricardovelero/nvidia_miner/master/files/settings.conf "-o ~/settings.conf
fi
...
while true; do
	...
	options=(1 "$MY_VISUDO_TITLE (DO IT FIRST!)"
	         2 "$MY_UPDATE_TITLE"
					 3 "$MY_PASSWD_TITLE"
	         4 "$MY_TIMEZONE_TITLE"
					 5 "$MY_KEYBOARD_TITLE"
